...
function VideoCall() {
  ...
  const initializeCall = async () => {
    try {
      ...
      // ── Auto-recording: start immediately on room join ──────────────────
      meetingStartRef.current = new Date();
      try {
        const mimeType = ['video/webm;codecs=vp9,opus', 'video/webm;codecs=vp8,opus', 'video/webm']
          .find(m => MediaRecorder.isTypeSupported(m)) || '';
        const autoRec = new MediaRecorder(stream, mimeType ? { mimeType } : {});
        autoRecordingChunksRef.current = [];
        autoRec.ondataavailable = (e) => {
          if (e.data && e.data.size > 0) autoRecordingChunksRef.current.push(e.data);
        };
        autoRec.start(1000);
        mediaRecorderRef.current = autoRec;
        setIsRecording(true);
        console.log('🔴 Auto-recording started');
      } catch (recErr) {
        console.warn('⚠️ Auto-recording failed to start:', recErr.message);
      }
  ...
  const saveMeetingHistory = useCallback((recordingsList) => {
    try {
      const endTime = new Date();
      const startTime = meetingStartRef.current || endTime;
      ...
      const entry = {
        id: Date.now(),
        roomId,
        participantName: location.state?.participantName || 'Unknown',
      ...
      console.warn('Could not save meeting history:', e);
    }
  }, [roomId, location.state, chatMessages]);

  const cleanup = useCallback((recordingsList) => {
    ...
    // Stop auto-recording and save meeting history
    const finalRecordings = recordingsList || [...recordings];
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      mediaRecorderRef.current.onstop = () => {
        if (autoRecordingChunksRef.current.length > 0) {
          const blob = new Blob(autoRecordingChunksRef.current, { type: 'video/webm' });
          const url = URL.createObjectURL(blob);
          const autoRec = {
            id: Date.now(),
            type: 'both',
            url,
            filename: `auto_recording_${roomId}_${Date.now()}.webm`,
            timestamp: new Date().toLocaleString(),
          };
          finalRecordings.push(autoRec);
        }
        saveMeetingHistory(finalRecordings);
      };
      mediaRecorderRef.current.stop();
      setIsRecording(false);
    } else {
  ...
  }, [recordings, saveMeetingHistory, roomId, stopContinuousTranslation]);
